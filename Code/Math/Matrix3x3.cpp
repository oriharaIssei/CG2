...
Matrix3x3 MakeMatrix3::Scale(const Vector2& scale) {
...
Matrix3x3 MakeMatrix3::Rotate(const float& theta) {
...
Matrix3x3 MakeMatrix3::Translate(const Vector2& t) {
...
Matrix3x3 MakeMatrix3::WorldMatrix(const Vector2& scale, const float& theta, const Vector2& localOrigin) {
	Matrix3x3 worldMatrix{};
	worldMatrix = MakeMatrix3::Scale(scale) * MakeMatrix3::Rotate(theta);
	worldMatrix *= MakeMatrix3::Translate(localOrigin); // localOrigin を考慮する
	return worldMatrix;
}

Matrix3x3 MakeMatrix3::OrthoMatrix(const float& left, const float& top, const float& right, const float& bottom) {
...
Matrix3x3 MakeMatrix3::ViewPortMatrix(float left, float top, float width, float height) {
	Matrix3x3 viewPort{
		width / 2.0f,0,0,
		0,-height / 2.0f,0,
...
Matrix3x3 MakeMatrix3::WvpVpMatrix(const Vector2& scale, const float& theta, const Vector2& localOrigin, const Vector2& worldCameraPos, const Vector2& windowLtPos, const Vector2& windowRbPos,const Vector2& viewPortOrigin, const Vector2& windowSize) {
	Matrix3x3 wvpVpMatrix{};
	Matrix3x3 camera = Translate(worldCameraPos);
	Matrix3x3 worldMatrix= MakeMatrix3::WorldMatrix(scale, theta, localOrigin);
	Matrix3x3 orthoMatrix = MakeMatrix3::OrthoMatrix(windowLtPos.x, windowLtPos.y, windowRbPos.x, windowRbPos.y);
	Matrix3x3 viewPortMatrix= MakeMatrix3::ViewPortMatrix(viewPortOrigin.x, viewPortOrigin.y, windowSize.x, windowSize.y);
...
Matrix3x3 MakeMatrix3::WvpVpMatrix(const Matrix3x3& worldMa, const Matrix3x3& viewMa, const Matrix3x3& orthoMa, const Matrix3x3& ViewPortMa) {
	Matrix3x3 wvpVpMatrix = (((worldMa * viewMa) * orthoMa)*ViewPortMa);
